Feature: Handle invalid Document ID and Project ID cases in the Doc Linking API
  ...
  Scenario Outline: Expect HTTP 404 when Project ID is invalid
    ...
    When method <method>
    Then status <status>
    And match /Error/ErrorCode == 'ENTITY_NOT_FOUND'
  ...
  Scenario Outline: Expect HTTP 404 when Project ID is not accessible to user
    Given path 'projects', '1879048004', 'documents', source_doc_id , 'relationships'
    And def user_not_on_project = call signIn { username: 'jdoe', password: 'ac0n3x72'}
  ...
  Scenario Outline: Expect HTTP 404 when Source Document ID is malformed
    Given path 'projects', '1879048004_malformed', 'documents', 'jklsbklj473586642038' , 'relationships'
    And request ''
    When method <method>
    Then status <status>
    And match /Error/ErrorCode == 'ENTITY_NOT_FOUND'
...
# 400 POST & DELETE
  ...
  Scenario Outline: Expect HTTP 400 when Target Document ID is invalid
    ...
    And match /Error/ErrorDescription == 'DocumentIds 271341877549073143 are not accessible or invalid'
  ...
  Scenario Outline: Expect HTTP 400 when Target Document ID is malformed
    ...
    Then status <status>
    And match /Error/ErrorCode == 'INVALID_REQUEST'
    And match /Error/ErrorDescription == 'DocumentIds 271341877_malformed are not accessible or invalid'
  ...
  Scenario Outline: Expect HTTP 400 when Target Document is not accessible to user
    ...
    Then status <status>
    And match /Error/ErrorCode == 'INVALID_REQUEST'
    And match /Error/ErrorDescription == 'DocumentIds 271341877549174594 are not accessible or invalid'
